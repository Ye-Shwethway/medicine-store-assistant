...
8. For a confirmed new expiry lot of an existing item, insert a real adjacent sibling row, suffix all participating sibling lot names as needed, set new-row `Remaining Stock` to `0`, put source quantity in `Received Stock`, preserve the verified local unit, and renumber `No.` sequentially without green markers.
9. Do not seed derived/helper fields during new-lot insertion. In particular, leave `Price` untouched because the Excel workflow derives it and may apply expiry-related pricing logic.
10. Green only direct verified source/configuration/identity writes; never green a whole inserted row or structural `No.` renumbering.
11. If Google Sheets write capability is unavailable, stop and say so. Do not claim that any update occurred.
12. Before mutating, state the source evidence and exact target cells. After mutating, read back values and formats and verify unrelated cells were not changed.
