  ...
  <main id="msa">
    <section class="view" data-panel="inventory">
      <div class="inventory-view-toolbar">
        <select id="inventoryPresetSelect"><option value="migration-review" selected>Migration Review</option><option value="cms-mapping-review">CMS Mapping Review</option></select>
      ...
      <table id="inventoryViewTable"><thead><tr>
        ...
        <th data-field="local_item_name" aria-sort="descending">Local Item</th>
...
await page.evaluate(()=>{
  ...
  document.addEventListener('click',event=>{
    const anchor=event.target.closest('a[href*="/dashboard/api/inventory-view/export.csv"]');
...
assert.ok(order.indexOf('inventoryExportCsv')<order.indexOf('inventoryViewRefresh'),'Export CSV should sit before Refresh');
await button.click();
...
assert.equal(url.pathname,'/dashboard/api/inventory-view/export.csv');
...
await browser.close();
console.log('inventory_csv_export_smoke=pass current_fields=pass filters=pass sort=pass source_scope=pass row_facts=false');
