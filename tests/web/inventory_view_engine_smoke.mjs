...
await page.evaluate(() => {
  ...
  window.fetch = async input => {
    ...
    if (url.startsWith('/dashboard/api/inventory-view/rows?')) {
      ...
      const source = preset === 'main-stock'
        ? { display_no: 1, local_item_name: '10cc Syringe', expiry_date: '2031-03-01', unit: 'Pcs', opening_qty: '120.000', current_qty: '120.000', cms_code: 'S10100667', mapping_status: 'REVIEW_REQUIRED' }
        : { source_row_no: 41, local_item_name: 'Bandage- Soft Bandage 6"', source_current_qty: '12.000', current_qty: '0.000', source_classification: 'REVIEW', review_reason: 'duplicate Product+Expiry source key' };
      const item = Object.fromEntries(columns.map(column => [column.field, source[column.field] ?? null]));
      return response({ view, columns, items: [item], count: 1, limit: 100, offset: 0, read_only: true, customizable_projection: true, database_canonical: false, migration_baseline_accepted: false });
    }
...
assert.ok(searchRequest.includes('q=bandage'));

await browser.close();
console.log('inventory_view_engine_browser_smoke=pass viewport=390x844 preset_switch=pass column_projection=pass');
