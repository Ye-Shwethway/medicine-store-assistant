...
def main() -> None:
    payload = build_excel_workbook(
        ExcelWorkbookSpec(
            ...
            columns=(
                ...
                ExcelColumn("qty", "Current Qty", "decimal", 14),
                ExcelColumn("expiry", "Expiry Date", "date", 14),
            ...
            rows=(
                ...
                {"item": "Normal Item", "qty": Decimal("3"), "expiry": None, "note": "@literal"},
            ),
        )
    )
    ...
    assert worksheet["D3"].value == "'@literal"
    assert worksheet["B2"].value == 12.5
    assert worksheet["B2"].number_format == "0.###"
    assert worksheet["C2"].number_format == "yyyy-mm-dd"
    ...
    assert table.ref == "A1:D3"
    assert table.autoFilter is not None
    print(
        "reusable_excel_export=pass xlsx=pass header_style=pass freeze_panes=pass "
        "wrap=pass sizing=pass borders=pass table_filter=pass typed_cells=pass formula_literal=pass"
    )
