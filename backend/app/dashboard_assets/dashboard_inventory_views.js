(()=>{
  ...
  function updateSelection(){selectionBar.hidden=state.selected.size===0;$('#inventorySelectionCount').textContent=`${state.selected.size.toLocaleString()} selected`;askAI.hidden=!isReviewPreset();askAI.disabled=state.selected.size>20;askAI.title=state.selected.size>20?'Select at most 20 rows for AI review.':'Build a read-only server-rehydrated review context.';tbody.querySelectorAll('tr[data-row-key]').forEach(tr=>tr.classList.toggle('inventory-row-selected',state.selected.has(tr.dataset.rowKey)))}
