(()=>{
  ...
  panel.innerHTML=`
    ...
    <div class="inventory-selection-bar" id="inventorySelectionBar" hidden><strong id="inventorySelectionCount">0 selected</strong><span>Review context only — no acceptance or mutation.</span><button class="secondary" id="inventoryAskAI" type="button">Ask AI</button><button class="secondary" id="inventoryDeepReview" type="button">Deep Review</button><button class="secondary" id="inventoryClearSelection" type="button">Clear</button></div>
  ...
  function categoryLabel(value){
    ...
    const known={
      CONTINUITY_EXACT_NAME_PRICE_SAME:'Continuity: exact name, same price',
      CONTINUITY_EXACT_NAME_PRICE_CHANGED:'Continuity: exact name, price changed',
      MULTIPLE_SOURCE_CODES:'Multiple CMS codes in source',
      CMS_DISCONTINUED:'CMS item discontinued; local item retained',
      CODE_NAME_MISMATCH:'CMS code/name mismatch',
      UNMAPPED:'No CMS mapping found',
      NEW_UNMAPPED:'New item without CMS mapping',
      MISSING_SOURCE_CMS_NAME:'Source CMS name is missing',
      MULTIPLE_SOURCE_CMS_NAMES:'Multiple source CMS names',
      RECYCLED_CODE:'CMS code appears recycled'
    };
    return known[key]||readableToken(key)||'Review evidence';
  }
  function reasonEvidence(value){
    if(value===null||value===undefined||value==='')return {summary:'—',details:[]};
    if(typeof value!=='string')return {summary:String(value),details:[]};
    const raw=value.trim();
    if(!(raw.startsWith('{')&&raw.endsWith('}')))return {summary:raw,details:[]};
    try{
      const parsed=JSON.parse(raw);
      if(!parsed||Array.isArray(parsed)||typeof parsed!=='object')return {summary:raw,details:[]};
      const category=parsed.category??parsed.reason_code??parsed.code;
      const preferred=parsed.message??parsed.reason??parsed.summary??parsed.detail;
      const summary=preferred?String(preferred):categoryLabel(category);
      const details=[];
      if(category)details.push(['Category',categoryLabel(category)]);
      Object.entries(parsed).forEach(([key,val])=>{
        if(['category','reason_code','code','message','reason','summary','detail'].includes(key)||val===null||val===undefined||val==='')return;
        const text=typeof val==='object'?(Array.isArray(val)?val.join(', '):Object.entries(val).map(([k,v])=>`${readableToken(k)}: ${v}`).join(' · ')):String(val);
        details.push([readableToken(key),text]);
      });
      return {summary,details};
    }catch{return {summary:raw,details:[]}}
  }
  ...
  function updateSelection(){const tooMany=state.selected.size>20;selectionBar.hidden=state.selected.size===0;$('#inventorySelectionCount').textContent=`${state.selected.size.toLocaleString()} selected`;askAI.hidden=!isReviewPreset();deepReview.hidden=!isReviewPreset();askAI.disabled=tooMany;deepReview.disabled=tooMany;const title=tooMany?'Select at most 20 rows for AI review.':'Build a read-only server-rehydrated review context.';askAI.title=title;deepReview.title=tooMany?'Select at most 20 rows for Deep Review.':'Prefill the existing Owner Multi-Agent REVIEW workspace without running it.';tbody.querySelectorAll('tr[data-row-key]').forEach(tr=>tr.classList.toggle('inventory-row-selected',state.selected.has(tr.dataset.rowKey)))}
  ...
  function buildAiDraft(context){const lines=[`Review these ${context.selected_count} selected rows from ${context.view.name}.`,'This is server-rehydrated shadow review evidence and is not canonical. Explain, compare, summarize and prioritize only; do not claim or perform mapping, price, inventory, baseline or canonical changes.','','Selected evidence:'];context.rows.forEach((row,index)=>{lines.push(`${index+1}. ${row.local_item_name||row.cms_name||`Row ${index+1}`}`);context.view.columns.forEach(column=>{let value=row[column.field];if(value===null||value===undefined||value==='')return;if(column.field==='review_reason')value=reasonEvidence(value).summary;lines.push(`- ${column.label}: ${value}`)});lines.push('')});return lines.join('\n').trim()}
  ...
  async function askSelectedRows(){if(!isReviewPreset()||!state.selected.size)return;askAI.disabled=true;const original=askAI.textContent;askAI.textContent='Preparing…';try{const context=await requestSelectedContext();const nav=root.querySelector('#aiWorkspaceNav'),input=root.querySelector('#aiMessageInput');if(!nav||!input)throw new Error('AI Workspace is not available in this dashboard');nav.click();input.value=buildAiDraft(context);input.dispatchEvent(new Event('input',{bubbles:true}));root.dispatchEvent(new CustomEvent('msa:inventory-review-context-ready',{detail:{context}}));setTimeout(()=>input.focus(),0)}catch(err){window.alert(err.message)}finally{askAI.textContent=original;askAI.disabled=state.selected.size>20}}
  async function deepReviewSelectedRows(){if(!isReviewPreset()||!state.selected.size)return;deepReview.disabled=true;const original=deepReview.textContent;deepReview.textContent='Preparing…';try{const context=await requestSelectedContext();root.dispatchEvent(new CustomEvent('msa:inventory-deep-review-context-ready',{detail:{context}}))}catch(err){window.alert(err.message)}finally{deepReview.textContent=original;deepReview.disabled=state.selected.size>20}}
