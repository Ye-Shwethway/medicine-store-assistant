(()=>{
  ...
  async function loadDiscussionTargets(workItemId){
    const select=document.querySelector('#aiMultiMode [data-discussion-target-for="'+CSS.escape(workItemId)+'"]');if(!select||select.dataset.targetsLoaded==='1')return;
    select.dataset.targetsLoaded='1';
    try{const data=await api('/dashboard/api/ai-workspace/multi-agent/work-items/'+encodeURIComponent(workItemId)+'/discussion-targets');if(!select.isConnected)return;select.innerHTML='<option value="">Default · @'+esc(data.default_call_name||'participant')+'</option>'+(data.items||[]).map(p=>'<option value="'+esc(p.call_name)+'">'+esc(p.display_name)+' · @'+esc(p.call_name)+' · '+esc(p.orchestration_role)+'</option>').join('')}catch(err){delete select.dataset.targetsLoaded;statusNotice(err.message,'error')}
  }
