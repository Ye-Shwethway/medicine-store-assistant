...
Inventory Sheet Formatting v1 is complete and production-verified through PR #203 / main `20f4b67ff79ea52838401c35ad19ad25da68c2a2`. It adds session-only fill formatting for cells/ranges/whole rows with a bounded six-color palette and Clear fill. Formatting is keyed by stable row identity + registered field, system REVIEW/CONFLICT signals remain visible, Copy TSV remains value-only, and Ask AI / Deep Review remain whole-row/server-rehydrated. Persistent formatting and styled custom-view export belong to Saved Custom Views / View Builder, not this slice. Production deploy run `32831427749` succeeded and deployment issue #26 confirmed `source_sha=20f4b67ff79ea52838401c35ad19ad25da68c2a2`.
...
## Inventory Fill runtime hotfix checkpoint

Production observation after Sheet Formatting v1 found two issues: user fill could be stored without an obvious visible cell color, and expanded color controls looked too much like normal native actions. PR #205 fixed both. Current production fill rendering uses authoritative inline background presentation with `important` priority; the Fill control has a distinct formatting marker; the palette uses derived swatch tiles rather than native `.secondary` buttons; `No fill` is a separated in-popover action; system warning stripes and selection outlines remain independently visible. PR #205 merged as `66a45963d5605f2699545cd35e24aa1dd2571152` and deploy run `32834067245` succeeded; deployment issue #26 confirms the exact source SHA. Formatting remains session-only/read-only and is not inventory or AI-review evidence.
